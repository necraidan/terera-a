...
@Component({
  ...
  template: `
    <ta-page-header [title]="sheet()?.title ?? 'Fiche introuvable'" backTo="/infos" />

    <main class="mx-auto max-w-md px-4 pb-28">
      @let current = sheet();

      @if (current) {
        <p class="mb-4 text-ink-2">{{ current.summary }}</p>
        ...
        @for (section of current.sections; track $index) {
          @switch (section.kind) {
            ...
            @case ('facts') {
              <dl class="mb-4 overflow-hidden rounded-card bg-surface-1">
