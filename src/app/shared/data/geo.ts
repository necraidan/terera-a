...
/**
 * Le dixième de pixel suffit largement pour un tracé vu à l'échelle 1 : au
 * delà, ce sont des octets perdus. Un tracé destiné à être zoomé demande
 * quelques décimales de plus, d'où le paramètre.
 */
const roundPx = (value: number, decimals: number): number => {
...
export function svgPathFromRings(
  ...
  for (const ring of rings) {
    ...
    for (const [lon, lat] of ring) {
      ...
      path += `${command}${roundPx(x, decimals)} ${roundPx(y, decimals)}`;
      command = 'L';
    }
    path += 'Z';
  }
